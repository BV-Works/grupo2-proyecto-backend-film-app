...
const addFavorite = async (req, res) => {
  try {
    ...
    if (!movieSource || !movieSourceId)
      return res
        .status(400)
        .send("Bad request: mandatory movieSource and movieSourceId");
    ...
    if (!validSources.includes(movieSource)) {
      return res
        .status(400)
        .json({ message: "movieSource must be omdb or mongo" }); // let user know of constraints
    }

    const [favorite, created] = await Favorite.findOrCreate({
      // favorite = movie object  created = boolean
      where: {
        userId: userId,
